...
2. Current : 최신 기능
    ...
    개발이 진행중인 버전
    해당 버전에 존재하는 기능이 패치를 통해 사라지거나 변경돠어
...
- npm global 설치로 pc에 의존성 라이브러리를 설치하는 건, 이후에 패치나 업데이트 등 변경사항이 있을때마다 삭제→ 설치 라는 번거로운 작업일 수반할 수 있음.
- 테스트가 빈번하게 일어나는 상황에서 일일이 설치, 실행, 제거를 반복하기 보다 1회성으로 실행하기 위한 도구가 있으면 좋겠다는 취지에서 나온 것이 npx
